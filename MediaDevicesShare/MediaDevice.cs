...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
        ...
        public IEnumerable<Item> EnumerateFileItems(string path, string searchPattern, SearchOption searchOption = SearchOption.TopDirectoryOnly)
        {
            if (path == null)
            {
                throw new ArgumentNullException(nameof(path));
            ...
            string pattern = MediaDevice.FilterToRegex(searchPattern);
            return item.GetChildren(pattern, searchOption).Where(i => i.Type == ItemType.File);
        }

        public IEnumerable<MediaFileInfo> EnumerateUsedMediaFilesInfo(string path, string searchPattern, SearchOption searchOption = SearchOption.AllDirectories)
        {
            ...
            foreach(var used_item in used_items)
            {
                ...
                {
                    var mediafileinfo = new MediaFileInfo(this, used_item);
                    if (string.IsNullOrEmpty(mediafileinfo.Artist) || string.IsNullOrEmpty(mediafileinfo.Title)) continue;
